...
struct LoginView<Model>: View where Model: ILoginViewModel {
    ...
    var body: some View {
        ...
        VStack {
            WelcomeText()
            UserImage()
            TextField(LoginConstants.enterUsername, text: $email)
                .padding()
                .background(lightGreyColor)
                .cornerRadius(cornerRadius)
                .padding(.bottom, ViewLayoutConstants.bottomPadding.rawValue)
                .foregroundColor(Color.black)
            SecureField(LoginConstants.enterPassword, text: $password)
                .padding()
                .background(lightGreyColor)
                .cornerRadius(cornerRadius)
                .foregroundColor(Color.black)
            ...
            Button(action: {
                ...
                ButtonContent(text: LoginConstants.login)
            }.padding()
        }
        .onAppear() {
            self.email = ""
...
struct WelcomeText: View {
    var body: some View {
        return Text(LoginConstants.welcomeMsg)
